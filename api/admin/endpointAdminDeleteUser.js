...
// Användarens forum, trådar och inlägg påverkas inte – endast användarnamnet ändras till "DeletedUser#X"
...
export default function softDeleteUserByAdmin(app, path, database) {
  app.patch(`${path}/admin/softdelete/:id`, async (request, response) => {
    ...
    if (!fakeAdmin || fakeAdmin.role !== "admin") {
      return response.status(403).json({ message: "Forbidden: Only admins can perform this action." });
    }
